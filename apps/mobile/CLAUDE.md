...
`(auth)/sign-in`. Every signed-in screen except the form modals (`trip/new`,
`trip/[id]/edit` — their headers carry only the ✕ close control) renders a
`SettingsCog` (`components/aurora/settings-cog.tsx`, imported directly — not
via the barrel) in its top-right header that pushes `/settings`. New screens consume the Aurora
