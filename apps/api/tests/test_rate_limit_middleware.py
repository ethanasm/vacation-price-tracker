...
def test_get_client_ip_takes_rightmost_trusted_hop():
    """With 1 trusted proxy hop (the default, matching the Cloudflare Tunnel),
    the client IP is the rightmost X-Forwarded-For entry — the one the trusted
    proxy observed — NOT the spoofable leftmost entry."""
...
def test_get_client_ip_ignores_spoofed_leftmost_entry():
    """A client prepending a forged IP can't move its own rate-limit bucket:
    with 1 trusted hop the forged leftmost entry is ignored."""
    spoofed = _make_request(
        headers={"X-Forwarded-For": "1.2.3.4, 198.51.100.2"}, client_host="10.0.0.9"
    )
    rotated = _make_request(
        headers={"X-Forwarded-For": "9.9.9.9, 198.51.100.2"}, client_host="10.0.0.9"
    )
    # Both requests come through the same real proxy-observed IP → same bucket.
...
def test_get_client_ip_malformed_forwarded_for_falls_back_to_peer():
    """A non-IP X-Forwarded-For value is rejected (not trusted, not logged) and
    the socket peer is used instead."""
...
def test_get_client_ip_zero_trusted_hops_ignores_forwarded_for(monkeypatch):
    """With trusted_proxy_count=0 (direct-bind deploy) a client-supplied XFF is
    ignored entirely in favour of the socket peer."""
